  ...
  case Session.getEntries session of
    ...
    Nothing ->
      ...
      , Http.send GotPackages <|
          Http.get (Href.join session.mountPoint "/search.json") (Decode.list Entry.decoder)
      )
...
view model =
  ...
  , attrs = []
  , kids =
      [ lazy2 (viewSearch model.session.mountPoint) model.query model.entries
      , viewSidebar model.session.mountPoint
      ]
  }
...
viewSearch : MountPoint -> String -> Entries -> Html Msg
viewSearch mount query entries =
  div [ class "catalog" ]
    [ input
        [ placeholder "Search"
        , value query
    ...
    , case entries of
        ...
        Success es ->
          let
            results =
              List.map (viewEntry mount) (Entry.search query es)
          in
          div []
            [ Keyed.node "div" [] <|
                ("h", viewHint mount (List.isEmpty results) query) :: results
            , p [ class "pkg-hint" ]
...
viewEntry : MountPoint -> Entry.Entry -> (String, Html msg)
viewEntry mount entry =
  ( entry.author ++ "/" ++ entry.project
  , lazy (viewEntryHelp mount) entry
  )


viewEntryHelp : MountPoint -> Entry.Entry -> Html msg
viewEntryHelp mount ({ author, project, summary } as entry) =
  div [ class "pkg-summary" ]
    [ div []
        ...
        , viewExactVersions mount entry
        ]
    , p [ class "pkg-summary-desc" ] [ text summary ]
    ]


viewExactVersions : MountPoint -> Entry.Entry -> Html msg
viewExactVersions mount entry =
  let
    latestUrl = Href.toVersion mount entry.author entry.project (Just entry.version)
    latestName = V.toString entry.version
    latestLink = a [ href latestUrl ] [ text latestName ]
  in
  span [ class "pkg-summary-hints" ] <|
    if V.toTuple entry.version == (1,0,0) then
      ...
      [ a [ href (Href.toProject mount entry.author entry.project) ] [ text "…" ]
      , text " "
      , latestLink
      ]
...
viewSidebar : MountPoint -> Html msg
viewSidebar mount =
  div [ class "catalog-sidebar" ]
    [ h2 [] [ text "Popular Packages" ]
    , ul [] <|
        List.map (viewPopularPackage mount) [ "core", "html", "json", "browser", "url", "http" ]
    , h2 [] [ text "Resources" ]
    , ul []
        ...
        , li [] [ a [ href (Href.join mount "/help/design-guidelines") ] [ text "API Design Guidelines" ] ]
        , li [] [ a [ href (Href.join mount "/help/documentation-format") ] [ text "Write great docs" ] ]
        , li [] [ a [ href "https://elm-lang.org" ] [ text "Elm Website" ] ]
        ]
    ]


viewPopularPackage : MountPoint -> String -> Html msg
viewPopularPackage mount project =
  li []
    [ a [ href (Href.toVersion mount "elm" project Nothing)
        ]
...
viewHint : MountPoint -> Bool -> String -> Html msg
viewHint mount noAlts query =
  viewHintHelp noAlts (String.toLower (String.replace "-" " " query)) (hints mount)
...
hints : MountPoint -> List (Hint msg)
hints mount =
  [ Hint "spa" 3 (singlePageApp mount)
  , Hint "single page app" 5 (singlePageApp mount)
  , Hint "components" 5 components
  , Hint "router" 4 (router mount)
  , Hint "routing" 4 (router mount)
  , Hint "routes" 4 (router mount)
  , Hint "focus" 4 (focus mount)
  , Hint "blur" 4 (focus mount)
  , Hint "scroll" 4 (scroll mount)
  , Hint "scrollheight" 7 (scroll mount)
  , Hint "scrollwidth" 7 (scroll mount)
  , Hint "scrollx" 7 (scroll mount)
  , Hint "scrolly" 7 (scroll mount)
  , Hint "scrollto" 7 (scroll mount)
  , Hint "scrollintoview" 7 (scroll mount)
  , Hint "mouse" 4 (mouse mount)
  , Hint "keyboard" 4 (keyboard mount)
  , Hint "window" 4 (window mount)
  , Hint "visibility" 5 (window mount)
  , Hint "animation" 5 (animation mount)
  , Hint "requestanimationframe" 8 (animation mount)
...
singlePageApp : MountPoint -> Html msg
singlePageApp mount =
  makeHint
    [ text "All single-page apps in Elm use "
    , codeLink (Href.toVersion mount "elm" "browser" Nothing) "elm/browser"
    , text " to control the URL, with help from "
    , codeLink (Href.toVersion mount "elm" "url" Nothing) "elm/url"
...
router mount =
  makeHint
    ...
    , codeLink (Href.toModule mount "elm" "browser" Nothing "Browser" (Just "application")) "Browser.application"
    , text " that manages the URL!"
    ]


focus : MountPoint -> Html msg
focus mount =
  makeHint
    [ text "Check out "
    , codeLink (Href.toModule mount "elm" "browser" Nothing "Browser.Dom" Nothing) "Browser.Dom"
    , text " for focusing on certain nodes. It uses tasks, so be sure you have learned about "
    , code [] [ text "Cmd" ]
    , text " values in "
    , guide
    , text " and then read through the "
    , codeLink (Href.toModule mount "elm" "core" Nothing "Task" Nothing) "Task"
    , text " module so you do not have to guess at how anything works!"
    ]


scroll : MountPoint -> Html msg
scroll mount =
  makeHint
    [ text "Check out "
    , codeLink (Href.toModule mount "elm" "browser" Nothing "Browser.Dom" Nothing) "Browser.Dom"
...
mouse : MountPoint -> Html msg
mouse mount =
  makeHint
    [ text "Folks usually use "
    , codeLink (Href.toModule mount "elm" "html" Nothing "Html.Events" Nothing) "Html.Events"
    , text " to detect clicks on buttons. If you want mouse events for the whole page, you may want "
    , codeLink (Href.toModule mount "elm" "browser" Nothing "Browser.Events" Nothing) "Browser.Events"
...
animation : MountPoint -> Html msg
animation mount =
  makeHint
    [ text "If you are not using CSS animations, you will need "
    , codeLink (Href.toModule mount "elm" "browser" Nothing "Browser.Events" (Just "onAnimationFrame")) "onAnimationFrame"
